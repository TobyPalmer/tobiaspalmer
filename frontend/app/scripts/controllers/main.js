...
angular.module('tobiaspalmerApp')
	.controller('MainCtrl', function ($scope, $http, $q, weatherService,$filter) {
		var Latitude = 58.59;	
		var Longitude = 16.18;
		...
		$scope.currentWeather = {};
   		...
   		$scope.frisbie = function(){
   			...
   			if($scope.currentWeather.t < tempLimit && $scope.currentWeather.ws > windLimit){
   				...
   				return "Det är ju tillräckligt varmt men din frisbie kommer blåsa iväg!"
   			}
   			else if($scope.currentWeather.t < tempLimit && $scope.currentWeather.ws <= windLimit){
   				return "Det är ju svinkallt men det blåser ju inte så mycket!"
   			}
   			else{
   				return "Det är ju perfekt frisbiegolfväder! Vad väntar du på?"
   			}
   		}
